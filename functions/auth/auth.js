...
import { createFirebaseTokenWithKakao } from "./kakao";
import { createFirebaseTokenWithNaver } from "./naver";
import { createFirebaseTokenWithGoogle } from "./google";
import { createFirebaseTokenWithApple } from "./apple";
    ...
    try {
        ...
        if (error.errorInfo && error.errorInfo.code && error.errorInfo.message) {
            ...
            if (provider == PROVIDER_KAKAO && errorData.code && errorData.msg) {
                ...
                msg = errorData.msg;
            } else if (provider == PROVIDER_NAVER && errorData.resultcode && errorData.message) {
                origin = provider.lowerCase();
                code = errorData.resultcode;
                msg = errorData.message;
            } else if (provider == PROVIDER_GOOGLE && errorData.error && errorData.error_description) {
                origin = provider.lowerCase();
                code = errorData.error;
                msg = errorData.error_description;
            } else if (provider == PROVIDER_APPLE && errorData.error && errorData.error_description) {
                origin = provider.lowerCase();
                code = errorData.error;
                msg = errorData.error_description;
            }
